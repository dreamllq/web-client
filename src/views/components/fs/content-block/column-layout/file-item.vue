<template>
  <div
    ...
    <template v-else>
      <text-tip :msg='`${f.name}.${f.fileDetail?.file?.ext}`' style='flex: 1;overflow: hidden;padding-left: 4px;' :enterable='false' />
    </template>
  </div>
</template>
...
const { onSelectF, onMultipleSelectF, onShiftMultipleSelectF, onFContextMenu, onEnterF } = useOperateHook();
