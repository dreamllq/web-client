...
import { BiViewMeta, BiViewMetaService } from '@/services/api';
...
const fetchData = async (option: { pageNo:number, pageSize:number }) => {
  const res = await BiViewMetaService.paginate({
    query: {
...
const onDelete = async ({ id }:{id:string}) => {
  ...
  await BiViewMetaService.remove({ path: { id } });
  ElMessage.success('删除成功');
  refresh();
};

const onSetting = (row:BiViewMeta) => {
  router.push({
