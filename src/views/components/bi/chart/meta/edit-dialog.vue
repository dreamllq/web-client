...
import { BiChartMeta, BiChartMetaService } from '@/services/api';
...
const onSubmit = async () => {
  ...
  await BiChartMetaService.update({
    path: { id },
    body: { ...data }
  });
  success = true;
...
const editDefaultData = ref<Partial<BiChartMeta>>({});
...
const edit = async (data:{ id:string }) => {
  ...
  const res = await BiChartMetaService.get({ path: { id } });
